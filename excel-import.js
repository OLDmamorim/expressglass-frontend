...
class ExcelImporter {
  ...
  async importData(processedData) {
    ...
    // ── LIMPAR serviços sem data que NÃO estão no Excel ──────────
    const excelPlates = new Set(
      processedData.map(s => String(s.plate).toUpperCase().trim())
    );

    const toDelete = (window.appointments || []).filter(a =>
      !a.date && !excelPlates.has(String(a.plate).toUpperCase().trim())
    );

    results.deleted = 0;
    for (const appt of toDelete) {
      try {
        await window.apiClient.deleteAppointment(appt.id);
        results.deleted++;
        results.details.push({ plate: appt.plate, status: 'deleted', reason: 'Não está no Excel — removido' });
      } catch (e) {
        console.warn('Erro ao apagar:', appt.plate, e.message);
      }
    }

    if (results.deleted > 0) {
      console.log(`🗑️ ${results.deleted} serviços removidos por não constarem no Excel`);
    }
