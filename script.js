...
function buildDaySummary(dayDate, isMobile) {
  ...
  // Tempo de regresso: valor real da otimização, ou estimar
  // Se não há rota calculada, usar totalTravelMin como estimativa (mesmo percurso de volta)
  const returnMin = lastItem?.return_time
    ? lastItem.return_time
    : returnKm > 0
      ? Math.round((returnKm / ROUTE_CONFIG.avgSpeedKmh) * 60)
      : Math.round(totalTravelMin * (hasOptimized ? 0.4 : 0.9)); // sem rota: regresso ≈ ida
