...
(function wireFormSubmitAndPlate(){
  ...
  document.addEventListener('click', function(e){
    ...
    if (form) {
      ...
      try { form.dispatchEvent(new Event('submit', { cancelable: true, bubbles: true })); } catch {}
    }
  }, true);

  // 4) Opcional: quando o modal abre, focar a matrícula e aplicar máscara inicial
  const origOpen = window.openAppointmentModal;
  window.openAppointmentModal = function(id){
    try { origOpen && origOpen.call(this, id); } catch(e) { console.error(e); }
    // pequeno atraso para garantir que o input existe no DOM
    setTimeout(() => {
      const plate = document.getElementById('appointmentPlate');
      if (plate) {
        plate.value = maskPlate(plate.value);
        try { plate.focus(); plate.setSelectionRange(plate.value.length, plate.value.length); } catch {}
      }
    }, 0);
  };
})();