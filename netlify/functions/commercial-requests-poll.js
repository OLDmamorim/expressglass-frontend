  ...
  window.crViewInAgenda = function(req) {
    ...
    if (addBtn) {
      ...
      setTimeout(function() {
        ...
        var hasComm = document.getElementById('hasCommercial');
        if (hasComm && !hasComm.checked) {
          hasComm.checked = true;
          hasComm.dispatchEvent(new Event('change'));
          // Aguardar loadComerciais e depois seleccionar
          setTimeout(function() {
            var sel = document.getElementById('appointmentCommercial');
            if (sel && r.commercial_id) {
              sel.value = r.commercial_id;
              if (!sel.value) {
